...
use App\Models\CompanyInfo;
	class Select {
		//creatinng a new instance of the select class
        ...
        function fill_list($category,$conditions =null,$category_id=null,$value=null,$attr=array())
        {
            ...
            {
              $query->where($column, $compare, $category_id[$key]);
            }
			if(isset($attr['ignore']))
            foreach($attr['ignore'] as $key=> $column)
            {
        ...
        function load_doctor($value=null)
        {
            $placeholder=$condition=array();
            return $this->fill_list('doctor',$placeholder,$condition,$value);
        }
        static function Get_currency_symbol($currency)
        {
			//get the currency symbol from the currency code using the currency array
            $currency_data = Select::instance()->currency_array();
	...
	{
		//creating a selection list of currencies from the currency array
		$html = '<select name="facility_currency" id="facility_currency" class="form-control" required>
	...
	{
		//creating a selection list of timezone from the timezone array
		$timezones =$this->Timezone_array();
		$html = '<select name="facility_timezone" id="facility_timezone" class="form-control" required>
	...
	{
		//list of currency with their associated code,name, symbol and country
		$currencies = array(
			array('code'=> 'ALL',
			'countryname'=> 'Albania',
			'name'=> 'Albanian lek',
	...
	{
		//list of timezones with the location,time difference and time zone code
		$timezones = array(
